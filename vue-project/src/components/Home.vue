...
export default {
  ...
  methods: {
    loadSuperHeroes: async function () {
      let superHeroesRequest = await fetch(
    ...
    filterSuperHeroes(superHeroes) {
      return superHeroes.filter(
        (p) =>
          p.name.toLowerCase().indexOf(this.superHeroSearch.toLowerCase()) !=
            -1 && !this.$root.favorites.find((q) => q.id == p.id)
      );
    },
  ...
  <main>
    <div class="home-container">
      <input
        type="text"
        placeholder="search"
        v-on:keyup="searchSuperHeroes()"
        v-model="superHeroSearch"
      />
      <div
          ...
          <div class="card-stats-container">
            <section class="stat-bar">
              ...
              <div class="colorless-bar">
                <div
                  class="attack-bar"
                  v-bind:style="{ width: superHero.powerstats.strength + '%' }"
                ></div>
              </div>
            </section>
            <section class="stat-bar">
